...
const MobileControls: React.FC = () => {
    ...
    return (
        <div className="absolute inset-0 pointer-events-none z-[60] flex flex-col justify-end p-6 pb-12">
            <div className="flex justify-between items-end w-full">
                {/* Left Side: Movement */}
                <div className="flex space-x-4 pointer-events-auto">
                    <button 
                        onPointerDown={(e) => { e.preventDefault(); dispatch('player-move-left'); }}
                        className="w-16 h-16 md:w-20 md:h-20 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl flex items-center justify-center active:scale-90 active:bg-white/20 transition-all shadow-lg"
                    >
                        <ChevronLeft className="w-8 h-8 md:w-10 md:h-10 text-white" />
                    </button>
                    <button 
                        onPointerDown={(e) => { e.preventDefault(); dispatch('player-move-right'); }}
                        className="w-16 h-16 md:w-20 md:h-20 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl flex items-center justify-center active:scale-90 active:bg-white/20 transition-all shadow-lg"
                    >
                ...
                <div className="flex flex-col space-y-4 items-end pointer-events-auto">
                    {hasImmortality && (
                        <button 
                            onPointerDown={(e) => { e.preventDefault(); dispatch('player-skill'); }}
                            className="w-16 h-16 md:w-20 md:h-20 bg-yellow-500/20 backdrop-blur-md border border-yellow-500/40 rounded-full flex items-center justify-center active:scale-90 active:bg-yellow-500/40 transition-all shadow-[0_0_15px_rgba(234,179,8,0.3)]"
                        >
                            <Shield className="w-8 h-8 md:w-10 md:h-10 text-yellow-400" />
                        </button>
                    )}
                    <button 
                        onPointerDown={(e) => { e.preventDefault(); dispatch('player-jump'); }}
                        className="w-20 h-20 md:w-24 md:h-24 bg-cyan-500/20 backdrop-blur-md border border-cyan-500/40 rounded-full flex items-center justify-center active:scale-90 active:bg-cyan-500/40 transition-all shadow-[0_0_20px_rgba(6,182,212,0.3)]"
                    >
                        <ArrowUp className="w-10 h-10 md:w-12 md:h-12 text-cyan-400" />
                    </button>
                </div>
            </div>
        </div>
                ...
                <div className="relative w-full bg-gray-900">
                     ...
                     <div className="absolute inset-0 flex flex-col justify-end items-center p-6 pb-8 text-center z-10">
                        ...
                        <p className="text-cyan-400/60 text-[10px] md:text-xs font-mono mt-3 tracking-wider uppercase">
                            {isMobile ? '[ TAP BUTTONS TO MOVE ]' : '[ ARROWS / SWIPE TO MOVE ]'}
                        </p>
                     </div>
                </div>
              </div>
